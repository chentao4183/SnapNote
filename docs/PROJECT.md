...
核心卖点是 **智能标注**:用户在截图上拖一个框,一步生成「目标框 + 箭头 + 文字标签」组合。传统截图工具通常需要分别切换矩形、箭头和文字工具;StepMark 把这三个动作合成一个工作流,减少截图反馈时的操作成本。
...
智能标注可以使用矩形或椭圆目标形状。标签朝远离目标框的方向延伸,左侧标签使用右对齐输入,保证箭头锚点稳定。
